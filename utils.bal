...
# Used for circuit breaker timing and backoff calculations.
...
# + statusCode - HTTP response status code to evaluate.
# + return - True if retryable, else false.
...
# Idempotent methods are safe to retry (e.g., GET, PUT, DELETE, HEAD, OPTIONS).
# + method - HTTP method name to check.
# + return - True if idempotent, else false.
...
# + millis - Timeout value in milliseconds.
# + return - Timeout value as decimal seconds.
